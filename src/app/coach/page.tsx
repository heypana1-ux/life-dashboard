...
export default function CoachPage() {
  ...
  const list = (
    <div className="flex flex-col gap-1">
      <button
        onClick={newChat}
        className="mb-1 flex items-center gap-2 rounded-xl bg-[var(--accent-soft)] px-3 py-2 text-sm font-medium text-[var(--accent)] hover:brightness-105"
      >
        <MessageSquarePlus size={16} /> {t("New chat")}
      </button>
      ...
      {chats.map((c) => (
        <div
          key={c.id}
          className={clsx(
            "group flex items-center gap-1 rounded-xl px-2.5 py-2 text-sm",
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="card relative flex min-h-0 flex-1 flex-col overflow-hidden border-[var(--accent)]/30 !p-0">
          <div className="grad flex items-center gap-2.5 px-4 py-3 text-white">
            ...
            <button onClick={() => setListOpen((o) => !o)} className="rounded-lg bg-white/20 px-2.5 py-1.5 text-xs font-medium lg:hidden">
              {t("Chats")}
            </button>
          </div>
